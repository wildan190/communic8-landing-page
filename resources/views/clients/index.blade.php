                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($clients as $client)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                                        <td class="px-4 py-3">
                                            @if($client->logo)
                                                <img src="{{ asset('storage/'.$client->logo) }}" alt="logo" class="h-10">
                                            @else
                                                <span class="text-gray-400">No Logo</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 text-gray-900 dark:text-gray-100 font-medium">{{ $client->company_name }}</td>
                                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $client->industry }}</td>
                                        <td class="px-4 py-3 text-center">
                                            <div class="inline-flex gap-3">
                                                <a href="{{ route('clients.edit', $client) }}" class="text-yellow-600 dark:text-yellow-400 hover:underline">Edit</a>
                                                <form action="{{ route('clients.destroy', $client) }}" method="POST" onsubmit="return confirm('Hapus client ini?')">
                                                    @csrf
                                                    @method('DELETE')
